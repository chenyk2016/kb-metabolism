...
> 状态：已实现（2026-07-09 验收通过：31 测试全绿；真实库 361 篇补发 kb_id、日志 128 行迁移、0 行失联）
> 实现偏差：embeddings 未换主键，改为"hash 认亲改绑"实现纯移动零重算——效果等同、改动更小；升级当次因 frontmatter 写入导致一次性全量重算向量（已发生，属预期成本）
