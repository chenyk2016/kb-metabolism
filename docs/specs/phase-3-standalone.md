...
> 2026-07-08 · 状态：**已确认并实施**（用户拍板：开源给别人用） · 前置：阶段 0-2 已上线

## 实施勘误（相对下文原稿的两处改进）

1. **访问日志从 kb.db 搬到 `.kb/access.log.jsonl`（追加式纯文本）**。第一性推导的自然结果：唯一不可再生的数据也应该是文件——git 可 diff、跟 vault 走、换机不丢；sqlite 从"部分派生"变成"完全派生"，不变量更干净。`kb migrate` 负责从旧 kb.db 导入。
2. **开源化落地**：MIT License、英文 README（中文并存）、CLI 输出英文、npm 包名 `kb-metabolism`（bin: `kb`/`kbm`）、GitHub 公开仓库。
